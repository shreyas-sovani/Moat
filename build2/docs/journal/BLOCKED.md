...
- Wrap consumed 0.019 ETH. Remaining ~0.001 ETH native as of seed. Re-check `GET /api/analytics/spend-cap` before any payable KH write. 3.5 drill/approve/save were non-payable (`journal/run1/spend-cap.json` still 0.019 / 0.02). Do not wrap.

### B-012 | KH Telegram plugin missing bot token | OPEN 2026-09-14T20:35Z

- Live 3.5 `executeWorkflow` top-up node succeeded onchain (`0x821dbaef…1b8a`) but KH execution `status=error`: "Telegram bot token is required. Please configure it in the integration settings."
- Supervisor treats write-node success as run succeeded (notify-only failure). Gate 3.5 PASS without wrapping.
- Blocks a demo video that must show the Telegram message. Configure the bot token on KH org integration `m2ovhyo51qj0ixr3pl3dq` (operator, not code). Local `.env` `TELEGRAM_BOT_TOKEN` is not the KH plugin setting.

## CLOSED

### B-011 | Morpho plugin no Base Sepolia | CLOSED 2026-09-14T20:20Z

Live `createWorkflow` 422: `morpho/supply-collateral` networks `1|8453|11155111`, not `"84532"`; `assets` must be uint256. Workaround: `web3/write-contract` / `directContractCall`. Evidence `journal/3.4/kh-422-morpho-plugin.json`. 3.5 used this path (not `morpho/withdraw-collateral`).
